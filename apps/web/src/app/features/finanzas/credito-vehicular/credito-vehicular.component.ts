...
@Component({
  ...
  template: `
    <main class="max-w-2xl mx-auto py-10 px-4 space-y-8">
      <header>
        <h1 class="text-3xl font-bold text-gray-900">Simulador de Crédito Vehicular</h1>
        <p class="mt-2 text-gray-600 text-sm">
          Calcula tu cuota mensual y el cronograma completo para financiar tu vehículo con el sistema francés (cuota fija).
          Ingresa la TEA que te ofrece el banco o concesionario.
        </p>
      </header>

      <form [formGroup]="form" (ngSubmit)="calcular()" class="bg-white rounded-xl border border-gray-200 shadow-sm p-6 space-y-5">
        <app-calc-input label="Precio / monto a financiar" inputId="monto" prefix="S/"
          placeholder="30000" [required]="true" [min]="500" formControlName="monto" />

        <div class="grid grid-cols-2 gap-4">
          <app-calc-input label="Plazo" inputId="plazo" placeholder="60"
            hint="En meses (máx. 84)" [min]="1" [max]="84" formControlName="plazoMeses" />
          <app-calc-input label="TEA (%)" inputId="tea" placeholder="20"
        ...
        <div class="rounded-lg bg-blue-50 border border-blue-100 p-3 text-xs text-blue-700">
          El plazo máximo típico para créditos vehiculares en el mercado peruano es de 60 a 84 meses (5-7 años).
          Consulta el <a routerLink="/comparador-de-prestamos" class="underline">comparador de préstamos</a> para ver tasas reales.
        </div>
        ...
        <button type="submit" [disabled]="form.invalid || calculando()"
          ...
          {{ calculando() ? 'Calculando…' : 'Simular crédito vehicular' }}
        </button>
      </form>
...
export class CreditoVehicularComponent implements OnInit {
  ...
  readonly form = this.fb.group({
    monto:       [null as number | null, [Validators.required, Validators.min(500)]],
    plazoMeses:  [60, [Validators.required, Validators.min(1), Validators.max(84)]],
    tea:         [20, [Validators.required, Validators.min(0), Validators.max(1000)]],
  });

  ngOnInit() {
    this.seo.set({
      title: 'Simulador de Crédito Vehicular — Perú Calcula',
      description: 'Simula tu crédito vehicular: calcula la cuota mensual y el cronograma completo. Ingresa el monto, plazo y TEA del banco.',
    });
  ...
  calcular() {
    ...
    this.api.post<CreditoRespuesta>('/finanzas/credito-vehicular', this.form.value).subscribe({
      next: (res) => {
        ...
        this.analytics.track({ tipoEvento: 'completado', calculadoraSlug: 'credito-vehicular', modulo: 'finanzas' });
      },
      error: () => {
